...
| `VOICE_DEDUP_WINDOW` | `300` | 内容防抖窗口（秒）：窗口内与上一条语音**内容高度相似**的请求会被拒绝（防 LLM 流式输出连发相近语音）；0 表示不启用 |
| `VOICE_DEDUP_SIMILARITY` | `0.6` | 防抖相似度阈值（0~1）：文本规范化后与上一条语音相似度达到该值即拒绝；**互为子串（含相等）直接判重** |
...
- 相似度（`SequenceMatcher` 比率）达到 `VOICE_DEDUP_SIMILARITY`（默认 0.6）即拒绝；
- **互为子串（含完全相等）直接判重**——覆盖"先发半句、再发全句"这类流式截断场景；
- 判重窗口为上一条语音发出后的 `VOICE_DEDUP_WINDOW` 秒（默认 300），窗口外不比较；
- 拒绝不进入冷却计时，也不影响之后内容不同的正常请求；按频道独立记录。
...
可等待冷却结束或调小 `VOICE_COOLDOWN`。若日志是「内容防抖命中」，说明本次内容与上一条语音高度相似被去重，
属预期行为；确需发送相近内容可调低 `VOICE_DEDUP_SIMILARITY` 或缩短 `VOICE_DEDUP_WINDOW`。
