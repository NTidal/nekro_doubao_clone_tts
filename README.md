...
| `VOICE_DEDUP_WINDOW` | `300` | 内容防抖窗口（秒）：窗口内与上一条语音**内容高度相似**的请求会被**静默吞掉**（发送一个极短的空音频占位、对 LLM 返回成功，不影响本回合文字输出）；0 表示不启用 |
...
1. 发送一个**极短的空音频占位**（约 0.14 秒静音 MP3，用户侧几乎无感知，最多看到一条"无声音"的语音条）；
